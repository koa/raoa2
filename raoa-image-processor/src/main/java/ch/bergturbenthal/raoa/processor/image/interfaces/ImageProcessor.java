...
public class ImageProcessor {
  ...
  private static AlbumEntryData createAlbumEntry(
      ...
      final UUID albumId) {
    final String filename = gitFileEntry.getNameString();
    final ObjectId fileId = gitFileEntry.getFileId();
    return createAlbumEntry(albumId, fileId, filename, metadata, xmpMeta);
  }
  ...
  public void processImage(ProcessImageRequest request) {
    ...
    final Mono<Tuple3<ObjectId, File, Optional<XMPMeta>>> loadedData =
        album.flatMap(
            a ->
    ...
    final Mono<AlbumEntryData> result =
        loadedData.flatMap(
            TupleUtils.function(
                (entryId, file, optionalXMPMeta) -> {
                  ...
                  final Function3<
                          Metadata, BufferedImage, Optional<TiffOutputSet>, Mono<AlbumEntryData>>
                      metadataBufferedImageOptionalMonoFunction3 =
                          (metadata, image, optionalTiffOutputSet) -> {
                            ...
                            return Flux.fromStream(Arrays.stream(SCALES).boxed())
                                .flatMap(
                                    size -> {
                                      String targetFilename =
                                          request.getAlbumId().toString()
                                              + "/"
                                              + entryId.name()
                                              + "-"
                                              + size
                                              + ".jpg";
                                      File targetFile =
                                          new File(properties.getThumbnailDir(), targetFilename);
                                      if (targetFile.exists()) return Mono.empty();
                                      return asyncService.asyncMono(
                                          () -> {
                                            final File parentDir = targetFile.getParentFile();
                                            if (!parentDir.exists()) parentDir.mkdirs();
                                            final File tempFile =
                                                new File(parentDir, UUID.randomUUID().toString());
                                            try {
                                              ...
                                              if (optionalTiffOutputSet.isPresent()) {
                                                ...
                                                /*
                                                                                                for (TiffOutputDirectory directory :
                                                                                                    tiffOutputSet.getDirectories()) {
                                                                                                  Set<Integer> existingTags = new HashSet<>();
                                                                                                  List<Integer> removeTags = new ArrayList<>();
                                                                                                  for (TiffOutputField field :
                                                                                                      directory.getFields()) {
                                                                                                    if (existingTags.contains(field.tag)) {
                                                                                                      removeTags.add(field.tag);
                                                                                                    }
                                                                                                    existingTags.add(field.tag);
                                                                                                  }
                                                                                                  for (int removeTag : removeTags) {
                                                                                                    directory.removeField(removeTag);
                                                                                                  }
                  ...
                  final Mono<AlbumEntryData> albumEntryDataMono =
                      Mono.zip(metadataMono, inputImage, overrideableMetadata)
                          .flatMap(TupleUtils.function(metadataBufferedImageOptionalMonoFunction3));

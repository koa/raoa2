...
public class InitAdminUserIfMissing {
    ...
    public InitAdminUserIfMissing(final Properties viewerProperties, final DataViewService dataViewService,
    ...
    public void checkAndFixAdminUser() {
        try {
            ...
            if (superuser == null)
                return;
            final AuthenticationId superUserId = AuthenticationId.builder().authority("accounts.google.com")
                    .id(superuser).build();
            final Mono<List<User>> existingSuperusers = dataViewService.findUserForAuthentication(superUserId)
                    .collectList();
            final Updater.CommitContext context = Updater.CommitContext.builder().message("set superuser by config")
                    .build();
            final Optional<User> createdUser = existingSuperusers.flatMap(maybeUser -> {
                if (maybeUser.size() > 0) {
                    final User existingUser = maybeUser.get(0);
                    final Mono<User> modifiedUser;
                    if (!existingUser.isSuperuser()) {
                        modifiedUser = userManager.context(existingUser.getId(),
                                user -> user.toBuilder().superuser(true).build(), context);
                    } else
                        modifiedUser = Mono.empty();
                    if (maybeUser.size() > 1) {
                        final List<User> additionalUsers = maybeUser.subList(1, maybeUser.size());
                        return Flux.fromIterable(additionalUsers).map(User::getId)
                                .flatMap(id -> userManager.removeUser(id, context)
                                        .doOnNext((done) -> log.info("Removed duplicate user " + id + ", " + done)), 1)
                                .then(modifiedUser);
                    }
                    return modifiedUser;
                } else {
                    return dataViewService.getPendingRequest(superUserId)
                            .flatMap(baseRequest -> userManager.createNewUser(baseRequest, context));
                }
            }).flatMap(updatedUser -> dataViewService.updateUserData().cast(User.class).defaultIfEmpty(updatedUser))

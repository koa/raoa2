...
public class DefaultProcessor implements Processor {
            ...
            if (HAS_DCRAW && RAW_ENDINGS.contains(ending)) {
                synchronized (dcrawLock) {
                    final Process process = Runtime.getRuntime()
                            .exec(new String[] { "dcraw", "-c", file.getAbsolutePath() });
    ...
    public boolean run() {
        ...
        return Boolean.TRUE.equals(albumList.getAlbum(albumId)
                .flatMap(ga -> ga
                        .listFiles(findAny(jobFiles.stream().map(s1 -> URLDecoder.decode(s1, Charsets.UTF_8))
                                .map(filename1 -> filename1 + ".xmp").map(PathFilter::create)
                                .collect(Collectors.toUnmodifiableList())))
                        .collectMap(GitAccess.GitFileEntry::getNameString, Function.identity())
                        .doOnNext(metadatafiles -> {
                            log.info("found {} metadata files for {}", metadatafiles.size(), jobFiles.size());
                        })
                        .flatMap(metadataFiles -> ga
                                .listFiles(findAny(jobFiles.stream().map(s -> URLDecoder.decode(s, Charsets.UTF_8))
                                        .map(PathFilter::create).collect(Collectors.toUnmodifiableList())))
                                .flatMap(contentFile -> {
                                    final String filename = contentFile.getNameString();
                                    final String metadataFilename = filename + ".xml";
                                    return Mono.zip(readFileEntryToTemp(ga, contentFile)
                                            .flatMap(entry -> asyncService.asyncMono(() -> {
                                                BodyContentHandler handler = new BodyContentHandler();
                                                Metadata metadata = new Metadata();

                                                final Path path = entry.getT2().toPath();
                                                try (final TikaInputStream inputStream = TikaInputStream.get(path)) {
                                                    parser.parse(inputStream, handler, metadata);
                                                    return metadata;
                                                }
                                            }).map(metadata -> Tuples.of(entry.getT1(), entry.getT2(), metadata))),
                                            Mono.justOrEmpty(metadataFiles.get(metadataFilename))
                                                    .map(GitAccess.GitFileEntry::getFileId)
                                                    .flatMap(entry -> ga.readObject(entry)
                                                            .flatMap(loader -> asyncService.asyncMono(() -> {
                                                                try (final ObjectStream stream = loader.openStream()) {
                                                                    return XMPMetaFactory.parse(stream);
                                                                }
                                                            })).map(meta -> Tuples.of(Optional.of(entry),
                                                                    Optional.of(meta)))
                                                            .onErrorResume(ex -> {
                                                                log.warn("Cannot read file " + metadataFilename, ex);
                                                                return Mono.empty();
                                                            }))
                                                    .defaultIfEmpty(Tuples.of(Optional.empty(), Optional.empty())))
                                            .map(t -> Tuples.of(t.getT1().getT1(), t.getT1().getT2(), t.getT1().getT3(),
                                                    t.getT2().getT1(), t.getT2().getT2()))
                                            .filterWhen(params -> {
                                                ...
                                                final Stream<ThumbnailFilenameService.FileAndScale> fileAndScaleStream = thumbnailFilenameService
                                                        .listThumbnailsOf(albumId, imageFileId);
                                                if (contentType.startsWith("image")) {
                                                    ...
                                                    if (!missingThumbnails.isEmpty()) {
                                                        log.info("Scaling " + filename + " to sizes "
                                                                + missingThumbnails.stream().map(
                                                                        ThumbnailFilenameService.FileAndScale::getSize)
                                                                        .map(Object::toString)
                                                                        .collect(Collectors.joining(", ")));
                                                        return asyncService.asyncMono(() -> {
                                                            final File mediaFile = params.getT2();

                                                            final ImageMetadata imageMetadata = Imaging
                                                                    .getMetadata(mediaFile);

                                                            final Optional<TiffOutputSet> tiffOutputSet;
                                                            if (imageMetadata instanceof JpegImageMetadata) {
                                                                tiffOutputSet = Optional.ofNullable(
                                                                        ((JpegImageMetadata) imageMetadata).getExif())
                                                                        .map(m -> {
                                                                            try {
                                                                ...
                                                                /*
                                                                 * } else if (imageMetadata instanceof
                                                                 * TiffImageMetadata) { tiffOutputSet =
                                                                 * Optional.ofNullable( ((TiffImageMetadata)
                                                                 * imageMetadata) .getOutputSet());
                                                                 */
                                                            } else
                                                    ...
                                                    final List<ThumbnailFilenameService.FileAndScale> missingThumbnails = fileAndScaleStream
                                                            ...
                                                            .collect(Collectors.toList());
                                                    if (!missingThumbnails.isEmpty())
                                                        return createVideoThumbnails(params.getT1().getNameString(),
                                                                params.getT2(), metadata, missingThumbnails);
                                                }
    ...
    private Mono<Tuple2<GitAccess.GitFileEntry, File>> readFileEntryToTemp(final GitAccess ga,
        ...
        return ga.readObject(fileEntry.getFileId()).flatMap(loader -> asyncService.asyncMono(() -> {
            final File tempFile = File.createTempFile("tmp", fileEntry.getNameString().replace('/', '_'));
    ...
    private boolean createImageThumbnails(final UUID albumId, final ObjectId entryId, final BufferedImage image,
            final Optional<TiffOutputSet> optionalTiffOutputSet,
            final Collection<ThumbnailFilenameService.FileAndScale> remainingFiles, final int orientation)
            throws ImageWriteException, IOException, ImageReadException {
        final int width = image.getWidth();
        final int height = image.getHeight();
        int maxLength = Math.max(width, height);
        final Stream<ThumbnailFilenameService.FileAndScale> s = thumbnailFilenameService.listThumbnailsOf(albumId,
                entryId);
        for (ThumbnailFilenameService.FileAndScale fileAndScale : remainingFiles) {
                ...
                continue;
            {
                ...
                try {
                    ...
                    BufferedImage targetImage = new BufferedImage(targetWith, targetHeight, BufferedImage.TYPE_INT_RGB);
                    ...
                    if (optionalTiffOutputSet.isPresent()) {
                        ...
                        if (tiffOutputDirectory != null) {
                            ...
                            tiffOutputDirectory.add(TiffTagConstants.TIFF_TAG_ORIENTATION,
                        ...
                        if (writeOk) {
                            {
                                ...
                                final OutputStream os = new BufferedOutputStream(new FileOutputStream(tempFile),
                                        256 * 1024);
                                new ExifRewriter().updateExifMetadataLossless(baos.toByteArray(), os, tiffOutputSet);
                            }

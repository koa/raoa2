...
public class ClientVersionUtil {
   ...
   *
   * @param version git client version string, of the form "git/1.7.9".
   * @return components of the version string. {@link #invalidVersion()} if the version string
   *     cannot be parsed.
   */
  ...
  private static int[] splitVersion(String versionString) {
    ...
    for (int i = 0; i < str.length; i++) {
      ...
      if ('0' <= c && c <= '9') {
        ...
        if (end == ver.length) ver = grow(ver);
        ver[end++] = acc;
        acc = 0;
   ...
   *
   * @param a first parsed version string.
   * @param b second parsed version string.
   ...
   *
   * @param ver the parsed version array.
   ...
   *
   * <p>These buggy clients do not display the status report from a failed push over HTTP.
   *
   * @param version parsed version of the Git client software.
   ...
   *
   * <p>Git 1.7.5 contains a unique bug where chunked requests are malformed. This applies to both
   * fetch and push.
   *
   * @param version parsed version of the Git client software.

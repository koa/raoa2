...
public class AlbumEntryQuery {
    ...
    public String entryUri(AlbumEntry entry) {
        return entry.getAlbum().getContext().getContexRootPath() + "/rest/album/" + entry.getAlbum().getId().toString()
                + "/" + entry.getId();
    }
    ...
    public OffsetDateTime created(AlbumEntry entry) {
        return Optional.ofNullable(extractDataEntryValue(entry, AlbumEntryData::getCreateTime))
                .map(i -> i.atOffset(ZoneOffset.UTC)).orElse(null);
    }

    public <T> T extractDataEntryValue(final AlbumEntry entry, final Function<AlbumEntryData, T> function) {

...
public class GitSmartHttpTools {
  ...
  public static final List<String> VALID_SERVICES =
      Collections.unmodifiableList(Arrays.asList(new String[] {UPLOAD_PACK, RECEIVE_PACK}));
  ...
  private static final List<String> SERVICE_SUFFIXES =
      Collections.unmodifiableList(
          Arrays.asList(new String[] {INFO_REFS_PATH, UPLOAD_PACK_PATH, RECEIVE_PACK_PATH}));
   ...
   *
   * <p>Server implementors may use this method to send customized error messages to a Git protocol
   * client using an HTTP 200 OK response with the error embedded in the payload. If the request was
   * not issued by a Git client, an HTTP response code is returned instead.
   *
   ...
   */
  public static void sendError(HttpServletRequest req, HttpServletResponse res, int httpStatus)
      throws IOException {
   ...
   * not issued by a Git client, an HTTP response code is returned instead.
   *
   * <p>This method may only be called before handing off the request to {@link
   * org.eclipse.jgit.transport.UploadPack#upload(java.io.InputStream, OutputStream, OutputStream)}
   * or {@link ReceivePack#receive(java.io.InputStream, OutputStream, OutputStream)}.
   *
   * @param req current request.
   * @param res current response.
   * @param httpStatus HTTP status code to set if the client is not a Git client.
   * @param textForGit plain text message to display on the user's console. This is shown only if
   *     the client is likely to be a Git client. If null or the empty string a default text is
   *     chosen based on the HTTP response code.
   * @throws IOException the response cannot be sent.
   */
  public static void sendError(
      HttpServletRequest req, HttpServletResponse res, int httpStatus, String textForGit)
      throws IOException {
    ...
    if (isInfoRefs(req)) {
      ...
      if (httpStatus < 400) ServletUtils.consumeRequestBody(req);
      res.sendError(httpStatus, textForGit);
    }
  }

  private static void sendInfoRefsError(
      HttpServletRequest req, HttpServletResponse res, String textForGit, int httpStatus)
      throws IOException {
  ...
  private static void sendReceivePackError(
    ...
    boolean sideband;
    ReceivePack rp =
        (ReceivePack) req.getAttribute("org.eclipse.jgit.transport.UploadPackOrReceivePack");
    if (rp != null) {
      try {
    ...
    } else sideband = isReceivePackSideBand(req);

    if (sideband) writeSideBand(buf, textForGit);
    else writePacket(pckOut, textForGit);
  ...
  private static void send(
      HttpServletRequest req, HttpServletResponse res, String type, byte[] buf, int httpStatus)
      throws IOException {
   ...
   *
   * <p>This method should only be invoked if {@link #isGitClient(HttpServletRequest)} is true.
   *
   * @param req current request.
   * @return the Content-Type the client expects.
   * @throws IllegalArgumentException the request is not a Git client request. See {@link
   *     #isGitClient(HttpServletRequest)}.
   */
  public static String getResponseContentType(HttpServletRequest req) {
    if (isInfoRefs(req)) return infoRefsResultType(req.getParameter("service"));
    else if (isUploadPack(req)) return UPLOAD_PACK_RESULT_TYPE;
    else if (isReceivePack(req)) return RECEIVE_PACK_RESULT_TYPE;
   ...
   *
   * <p>Generally the suffix is stripped by the {@code SuffixPipeline} handling the request, so this
   * method is rarely needed.
   *
   * @param path the path of the request.
   * @return the path up to the last path component before the service suffix; the path as-is if it
   *     contains no service suffix.
   */
  ...
  public static boolean isReceivePack(HttpServletRequest req) {
    ...
    return uri != null
        && uri.endsWith(RECEIVE_PACK_PATH)

...
public class QueryContextSupplier {
    ...
    public Mono<QueryContext> createContext() {
        ...
        final Optional<AuthenticationId> authenticationId = authorizationManager.currentAuthentication(context);
        final RequestAttributes requestAttributes = RequestContextHolder.getRequestAttributes();
        return authorizationManager.currentUser(context).cache().map(Optional::of).defaultIfEmpty(Optional.empty())
                .map(user -> new QueryContext() {

                    private final Mono<AuthenticationState> currentAuthenticationState = findCurrentAuthenticationState(
                            user, context);
                    ...
                    public String getContexRootPath() {
                        if (requestAttributes instanceof ServletRequestAttributes) {
                            ...
                            return currentRequestPath.substring(0, currentRequestPath.length() - servletPath.length());
                        } else
                            return "";
                    }
                    ...
                    public boolean canAccessGroup(final UUID groupId) {
                        return user.map(user1 -> user1.isSuperuser() || user1.getGroupMembership().contains(groupId))
                                .orElse(false);
                    }
                });
    }
    ...
    @NotNull
    private Mono<AuthenticationState> findCurrentAuthenticationState(final Optional<User> u,
            final SecurityContext context) {
        if (u.isPresent()) {
            return Mono.just(AuthenticationState.AUTHORIZED);
        }
        return authorizationManager.hasPendingRequest(context).onErrorReturn(false).map(hasPendingRequest -> {

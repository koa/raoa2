                  ...
                  .flatMap(
                      album ->
                          authorizationManager
                              .canUserAccessToAlbum(
                                  queryContext.getSecurityContext(), album.getRepositoryId())
                              .map(canAccess -> Tuples.of(album, canAccess)))
                  .<Optional<AlbumUpdatedEvent>>map(
                      t -> {
                        final AlbumData album = t.getT1();
                        final Boolean visible = t.getT2();
                        final UUID repositoryId = album.getRepositoryId();
                        if (visible) {
                          final ObjectId versionBefore =
                              lastShownVersion.put(repositoryId, album.getCurrentVersion());
                          if (versionBefore == null)
                            return Optional.of(
                                new AlbumUpdatedEvent(
                                    AlbumUpdatedEvent.AlbumUpdateEventType.ADDED,
                                    repositoryId,
                                    new Album(repositoryId, queryContext, Mono.just(album))));
                          else if (versionBefore.equals(album.getCurrentVersion())) {
                            return Optional.empty();
                          } else {
                            return Optional.of(
                                new AlbumUpdatedEvent(
                                    AlbumUpdatedEvent.AlbumUpdateEventType.MODIFIED,
                                    repositoryId,
                                    new Album(repositoryId, queryContext, Mono.just(album))));
                          }
                          ...
                          if (wasVisible != null) {
                            return Optional.of(
                                new AlbumUpdatedEvent(
                                    AlbumUpdatedEvent.AlbumUpdateEventType.REMOVED,
                                    repositoryId,
                                    null));

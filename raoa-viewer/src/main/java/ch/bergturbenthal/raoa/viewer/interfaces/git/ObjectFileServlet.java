...
abstract class ObjectFileServlet extends HttpServlet {
  ...
  /** {@inheritDoc} */
  @Override
  protected void doHead(final HttpServletRequest req, final HttpServletResponse rsp)
      throws ServletException, IOException {
    serve(req, rsp, false);
  }

  private void serve(
      final HttpServletRequest req, final HttpServletResponse rsp, final boolean sendBody)
      throws IOException {
    ...
    try {
      ...
      if (etag != null) rsp.setHeader(HDR_ETAG, etag);
      if (0 < lastModified) rsp.setDateHeader(HDR_LAST_MODIFIED, lastModified);

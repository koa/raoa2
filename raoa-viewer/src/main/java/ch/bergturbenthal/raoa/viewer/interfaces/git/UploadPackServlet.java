...
import static ch.bergturbenthal.raoa.viewer.interfaces.git.GitSmartHttpTools.UPLOAD_PACK;
import static ch.bergturbenthal.raoa.viewer.interfaces.git.GitSmartHttpTools.UPLOAD_PACK_REQUEST_TYPE;
import static ch.bergturbenthal.raoa.viewer.interfaces.git.GitSmartHttpTools.UPLOAD_PACK_RESULT_TYPE;
...
import static ch.bergturbenthal.raoa.viewer.interfaces.git.ServletUtils.consumeRequestBody;
import static ch.bergturbenthal.raoa.viewer.interfaces.git.ServletUtils.getInputStream;
...
class UploadPackServlet extends HttpServlet {
  ...
  static class InfoRefs extends SmartServiceInfoRefs {
    ...
    @Override
    protected void advertise(HttpServletRequest req, PacketLineOutRefAdvertiser pck)
        throws IOException, ServiceNotEnabledException, ServiceNotAuthorizedException {
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse rsp) throws IOException {
    ...
    UploadPackErrorHandler.UploadPackRunnable r =
        () -> {
          ...
          SmartOutputStream out =
              new SmartOutputStream(req, rsp, false) {
                @Override
  ...
  private void defaultUploadPackHandler(
      HttpServletRequest req, HttpServletResponse rsp, UploadPackErrorHandler.UploadPackRunnable r)
      throws IOException {
    try {
      r.upload();
    } catch (ServiceMayNotContinueException e) {
    ...
    getServletContext()
        .log(
            MessageFormat.format(
                HttpServerText.get().internalErrorDuringUploadPack, ServletUtils.identify(git)),
            e);
  }
}

...
import static ch.bergturbenthal.raoa.viewer.interfaces.git.GitSmartHttpTools.RECEIVE_PACK;
import static ch.bergturbenthal.raoa.viewer.interfaces.git.GitSmartHttpTools.RECEIVE_PACK_REQUEST_TYPE;
import static ch.bergturbenthal.raoa.viewer.interfaces.git.GitSmartHttpTools.RECEIVE_PACK_RESULT_TYPE;
...
class ReceivePackServlet extends HttpServlet {
  ...
  private void log(Repository git, Throwable e) {
    getServletContext()
        .log(
            MessageFormat.format(
                HttpServerText.get().internalErrorDuringReceivePack, ServletUtils.identify(git)),
            e);
  }
}

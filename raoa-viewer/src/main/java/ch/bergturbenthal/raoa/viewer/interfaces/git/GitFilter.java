...
import ch.bergturbenthal.raoa.viewer.interfaces.git.glue.ErrorServlet;
import ch.bergturbenthal.raoa.viewer.interfaces.git.glue.MetaFilter;
import ch.bergturbenthal.raoa.viewer.interfaces.git.glue.RegexGroupFilter;
import ch.bergturbenthal.raoa.viewer.interfaces.git.glue.ServletBinder;
import ch.bergturbenthal.raoa.viewer.interfaces.git.resolver.AsIsFileService;
import ch.bergturbenthal.raoa.viewer.interfaces.git.resolver.DefaultReceivePackFactory;
import ch.bergturbenthal.raoa.viewer.interfaces.git.resolver.DefaultUploadPackFactory;
 ...
 *
 * <p>Applications embedding this filter should map a directory path within the application to this
 * filter. For a servlet version, see {@link GitServlet}.
 *
 * <p>Applications may wish to add additional repository action URLs to this servlet by taking
 * advantage of its extension from {@link MetaFilter}. Callers may register their own URL suffix
 * translations through {@link #serve(String)}, or their regex translations through {@link
 * #serveRegex(String)}. Each translation should contain a complete filter pipeline which ends with
 * the HttpServlet that should handle the requested action.
 */
public class GitFilter extends MetaFilter {
   ...
   *
   * <p>The required parameter {@code base-path} must be configured to point to the local filesystem
   * directory where all served Git repositories reside.
   */
  public GitFilter() {
   ...
   *
   * @param resolver the resolver to use when matching URL to Git repository. If null the {@code
   *     base-path} parameter will be looked for in the parameter table during init, which usually
   *     comes from the {@code web.xml} file of the web application.
   */
   ...
   * Set AsIsFileService
   *
   * @param f the filter to validate direct access to repository files through a dumb client. If
   *     {@code null} then dumb client support is completely disabled.
   */
   ...
   *
   * @param f the factory to construct and configure an {@link
   *     org.eclipse.jgit.transport.UploadPack} session when a fetch or clone is requested by a
   *     client.
   */
  @SuppressWarnings("unchecked")
  public void setUploadPackFactory(UploadPackFactory<HttpServletRequest> f) {
    assertNotInitialized();
    this.uploadPackFactory =
        f != null ? f : (UploadPackFactory<HttpServletRequest>) UploadPackFactory.DISABLED;
  }

  /**
   * Set a custom error handler for git-upload-pack.
   *
   * @param h A custom error handler for git-upload-pack.
   * @since 5.6
   */
   ...
   *
   * @param filter filter to apply before any of the UploadPack operations. The UploadPack instance
   *     is available in the request attribute {@link ServletUtils#ATTRIBUTE_HANDLER}.
   */
   ...
   *
   * @param f the factory to construct and configure a {@link
   *     org.eclipse.jgit.transport.ReceivePack} session when a push is requested by a client.
   */
  @SuppressWarnings("unchecked")
  public void setReceivePackFactory(ReceivePackFactory<HttpServletRequest> f) {
    assertNotInitialized();
    this.receivePackFactory =
        f != null ? f : (ReceivePackFactory<HttpServletRequest>) ReceivePackFactory.DISABLED;
  }

  /**
   * Set a custom error handler for git-receive-pack.
   *
   * @param h A custom error handler for git-receive-pack.
   * @since 5.7
   */
   ...
   *
   * @param filter filter to apply before any of the ReceivePack operations. The ReceivePack
   *     instance is available in the request attribute {@link ServletUtils#ATTRIBUTE_HANDLER}.
   */
  ...
  public void init(FilterConfig filterConfig) throws ServletException {
    ...
    if (uploadPackFactory != UploadPackFactory.DISABLED) {
      ...
      for (Filter f : uploadPackFilters) b = b.through(f);
    ...
    if (receivePackFactory != ReceivePackFactory.DISABLED) {
      ...
      for (Filter f : receivePackFilters) b = b.through(f);
    ...
    } else refs.with(new ErrorServlet(HttpServletResponse.SC_NOT_ACCEPTABLE));
  ...
  private static File getFile(FilterConfig cfg, String param) throws ServletException {
    ...
    if (!path.exists())
      throw new ServletException(
          MessageFormat.format(HttpServerText.get().pathForParamNotFound, path, param));
    return path;
  }
  ...
  private static boolean getBoolean(FilterConfig cfg, String param) throws ServletException {
    ...
    try {
      ...
      throw new ServletException(
          MessageFormat.format(HttpServerText.get().invalidBoolean, param, n), err);
    }
  }
  ...
  protected ServletBinder register(ServletBinder binder) {
    if (resolver == null) throw new IllegalStateException(HttpServerText.get().noResolverAvailable);

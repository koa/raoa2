...
public class ResourceServerConfig extends ResourceServerConfigurerAdapter {
    ...
    public void configure(final HttpSecurity http) throws Exception {
        ...
        http.cors().and().oauth2ResourceServer().bearerTokenResolver(request -> {
            // log.info("Stacktrace", new Throwable());
            final Optional<String> s = Optional.ofNullable(request.getCookies())
                    .flatMap(cookies -> Arrays.stream(cookies).filter(cookie -> "access_token".equals(cookie.getName()))
            ...
            return token;
        }).jwt().and().and().addFilter(new BasicAuthenticationFilter(authentication -> {
                ...
                return elasticSearchDataViewService
                        ...
                        .<Authentication> map(user -> {
                            final Collection<? extends GrantedAuthority> authorities = Collections.singletonList(
                                    new SimpleGrantedAuthority(user.isSuperuser() ? "SUPERUSER" : "USER"));
    ...
    public TokenStore tokenStore(ResourceServerProperties resource) {
        ...
        final UserAuthenticationConverter us = new UserAuthenticationConverter() {
            @Override
            ...
            public Authentication extractAuthentication(final Map<String, ?> map) {
                ...
                final OAuth2User principal = new DefaultOAuth2User(authorities, (Map<String, Object>) map, "name");
                final String id = map.get("sub").toString();
                final OAuth2AuthenticationToken oAuth2AuthenticationToken = new OAuth2AuthenticationToken(principal,
                        authorities, "google");

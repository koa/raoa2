...
public final class ServletUtils {
  ...
  public static final String ATTRIBUTE_HANDLER =
      "org.eclipse.jgit.transport.UploadPackOrReceivePack";
   ...
   * @throws IllegalStateException the repository was not set by the filter, the servlet is being
   *     invoked incorrectly and the programmer should ensure the filter runs before the servlet.
   ...
   *
   * <p>This method automatically inflates the input stream if the request {@code Content-Encoding}
   * header was set to {@code gzip} or the legacy {@code x-gzip}.
   *
  ...
  public static InputStream getInputStream(HttpServletRequest req) throws IOException {
    ...
    if (ENCODING_GZIP.equals(enc) || ENCODING_X_GZIP.equals(enc)) in = new GZIPInputStream(in);
    else if (enc != null)
      throw new IOException(
          MessageFormat.format(
              HttpServerText.get().encodingNotSupportedByThisLibrary, HDR_CONTENT_ENCODING, enc));
    return in;
  }
   ...
   *
   * <p>The text response is encoded in the Git character encoding, UTF-8.
   *
   * <p>If the user agent supports a compressed transfer encoding and the content is large enough,
   * the content may be compressed before sending.
   *
   * <p>The {@code ETag} and {@code Content-Length} headers are automatically set by this method.
   * {@code Content-Encoding} is conditionally set if the user agent supports a compressed transfer.
   * Callers are responsible for setting any cache control headers.
   *
  ...
  public static void sendPlainText(
      final String content, final HttpServletRequest req, final HttpServletResponse rsp)
      throws IOException {
   ...
   * <p>The {@code ETag} and {@code Content-Length} headers are automatically set by this method.
   * {@code Content-Encoding} is conditionally set if the user agent supports a compressed transfer.
   * Callers are responsible for setting {@code Content-Type} and any cache control headers.
   *
   * @param content to return to the user agent as this entity's body.
   * @param req the incoming request.
   * @param rsp the outgoing response.
  ...
  public static void send(
      byte[] content, final HttpServletRequest req, final HttpServletResponse rsp)
      throws IOException {
  ...
  static boolean acceptsGzipEncoding(String accepts) {
    ...
    while (b < accepts.length()) {
      ...
      if (term.equals(ENCODING_GZIP)) return true;
      b = e + 1;
    }
    return false;
  }

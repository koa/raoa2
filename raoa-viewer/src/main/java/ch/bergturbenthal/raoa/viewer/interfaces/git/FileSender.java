 ...
 *
 * <p>Supports a single byte range requested via {@code Range} HTTP header. This feature supports a
 * dumb client to resume download of a larger object file.
 */
final class FileSender {
    ...
    try {
      ...
      r =
          new FileNotFoundException(
              MessageFormat.format(HttpServerText.get().cannotGetLengthOf, path));
      r.initCause(e);
      throw r;
    }
  }
  ...
  void serve(final HttpServletRequest req, final HttpServletResponse rsp, final boolean sendBody)
      throws IOException {
  ...
  private boolean initRangeRequest(final HttpServletRequest req, final HttpServletResponse rsp)
      throws IOException {

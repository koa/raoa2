...
public class UserQuery {
    ...
    public UserQuery(final DataViewService dataViewService, final AuthorizationManager authorizationManager) {
    ...
    public Flux<Album> canAccess(UserReference user) {
        return getVisibleAlbums(user).map(a -> new Album(a.getRepositoryId(), user.getContext(), Mono.just(a)));
    }
    ...
    private Flux<UUID> getDirectVisibleAlbums(final UserReference user) {
        ...
        return dataViewService.findUserById(user.getId())
                // .log("user")
    ...
    public Mono<Album> newestAlbumCanAccess(UserReference user) {
        ...
        return getVisibleAlbums(user)
                .collect(Collectors.maxBy(Comparator.comparing(
                        albumData -> albumData.getCreateTime() == null ? Instant.MIN : albumData.getCreateTime())))
    ...
    public Mono<Boolean> canManageUsers(UserReference user) {
        if (Objects.equals(user.getId(), user.getContext().getCurrentUser().map(User::getId).orElse(null))) {
    ...
    public Flux<GroupMembershipReference> groups(UserReference user) {
        if (canShowUserDetails(user)) {
            return dataViewService.findUserById(user.getId()).flatMapIterable(User::getGroupMembership)
                    .map(membership -> {
                        final GroupReference groupReference = new GroupReference(membership.getGroup(),
                                user.getContext(), dataViewService.findGroupById(membership.getGroup()).cache());
                        return new GroupMembershipReference(membership.getFrom(), membership.getUntil(),
                                groupReference);
                    });
        }
        return Flux.empty();
    ...
    public Flux<TemporaryPassword> temporaryPasswords(UserReference user) {
        if (canShowUserDetails(user)) {
            return dataViewService.findTemporaryPasswordsByUser(user.getId()).map(TemporaryPassword::from);
        }
        return Flux.empty();
    }
}

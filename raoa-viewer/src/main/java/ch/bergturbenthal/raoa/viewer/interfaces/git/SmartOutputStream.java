...
import static ch.bergturbenthal.raoa.viewer.interfaces.git.ServletUtils.acceptsGzipEncoding;
 ...
 *
 * <p>If the response overflows the buffer, gzip is skipped and the response is streamed to the
 * client as its produced, most likely using HTTP/1.1 chunked encoding. This is useful for servlets
 * that produce mixed-mode content, where smaller payloads are primarily pure text that compresses
 * well, while much larger payloads are heavily compressed binary data. {@link UploadPackServlet} is
 * one such servlet.
 */
class SmartOutputStream extends TemporaryBuffer {
  ...
  SmartOutputStream(
      final HttpServletRequest req, final HttpServletResponse rsp, boolean compressStream) {
    super(LIMIT);
    this.req = req;
    this.rsp = rsp;

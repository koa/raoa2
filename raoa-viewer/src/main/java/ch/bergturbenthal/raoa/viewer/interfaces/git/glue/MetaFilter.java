 ...
 *
 * <p>Callers can create and configure a new processing pipeline by using one of the {@link
 * #serve(String)} or {@link #serveRegex(String)} methods to allocate a binder for a particular URL
 * pattern.
 *
 * <p>Registered filters and servlets are initialized lazily, usually during the first request. Once
 * initialized the bindings in this servlet cannot be modified without destroying the servlet and
 * thereby destroying all registered filters and servlets.
 */
public class MetaFilter implements Filter {
   ...
   *
   * @param path pattern to match.
   * @return binder for the passed path.
   */
  public ServletBinder serve(String path) {
    if (path.startsWith("*")) return register(new SuffixPipeline.Binder(path.substring(1)));
    throw new IllegalArgumentException(
        MessageFormat.format(HttpServerText.get().pathNotSupported, path));
  }

  /**
   * Construct a binding for a regular expression.
   *
   * @param expression the regular expression to pattern match the URL against.
   ...
   * Construct a binding for a regular expression.
   *
   * @param pattern the regular expression to pattern match the URL against.
   * @return binder for the passed expression.
   */
  ...
  public void destroy() {
    if (pipelines != null) {
      ...
      for (UrlPipeline p : pipelines) p.destroy(destroyed);
      pipelines = null;
    }
  }
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (p != null) p.service(req, res);
    else chain.doFilter(req, res);
  }

  private UrlPipeline find(HttpServletRequest req) throws ServletException {
    for (UrlPipeline p : getPipelines()) if (p.match(req)) return p;
    return null;
  }
   ...
   *
   * @param b the newly created binder.
   * @return binder for the caller, potentially after adding one or more filters into the pipeline.
   */
  ...
  private UrlPipeline[] createPipelines() throws ServletException {
    ...
    for (int i = 0; i < bindings.size(); i++) array[i] = bindings.get(i).create();

    Set<Object> inited = newIdentitySet();
    for (UrlPipeline p : array) p.init(servletContext, inited);
    return array;
  }
}
